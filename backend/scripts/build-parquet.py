...
if glob.glob(J):
    ...
    show(f"SELECT count(*) AS total_jobs, count(*) FILTER (is_open) AS open_jobs, count(*) FILTER (is_open AND length(content) > 800) AS open_with_body FROM read_parquet('{J}', union_by_name=true)")
...
elif from_r2 and publish:
    # LOW_DISK: the local copies are gone; a cheap summary from the bucket (no text columns: parquet column pruning keeps the read small)
    Jr, Br = r2.url(f"exports/{date_name}/jobs/*.parquet"), r2.url(f"exports/{date_name}/boards/*.parquet")
    show(f"SELECT count(*) AS boards, count(*) FILTER (last_status='ok') AS ok, count(*) FILTER (last_status='gone') AS gone, count(*) FILTER (last_status='error') AS error FROM read_parquet('{Br}', union_by_name=true)")
    show(f"SELECT ats, count(*) AS jobs, count(*) FILTER (is_open) AS open, count(DISTINCT slug) AS boards FROM read_parquet('{Jr}', union_by_name=true) GROUP BY ats ORDER BY jobs DESC")
    size = sum(sz for k, sz, _ in r2.list(f"exports/{date_name}/jobs/") if k.endswith(".parquet"))
    print(f"published exports/{date_name}/jobs/*.parquet ({size / 1e6:.0f} MB) and boards/*.parquet to the bucket")
